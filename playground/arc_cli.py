"""
Arc CLI — interface de linha de comando para a Arc blockchain.

Usa arc-devkit do PyPI como dependência. Cada subcomando acessa
um módulo diferente do toolkit: conexão, copilot, gas, debugger.
...
Uso:
    ...
    python arc_cli.py balance 0xAbC...
    python arc_cli.py gas 0xDest... 10.5
    python arc_cli.py debug 0xTxHash...
    python arc_cli.py codegen "criar agente que monitora saldo"
"""
...
app = typer.Typer(
    name="arc",
    help="Ferramentas Arc blockchain — powered by arc-devkit (PyPI).",
...
def ask(
    ...
    raw: bool = typer.Option(False, "--raw", help="Exibe texto puro sem formatação Markdown."),
) -> None:
...
def balance(
    ...
    """Exibe o saldo nativo de uma carteira Arc."""
    ...
    try:
        ...
        saldo = Decimal(str(w3.from_wei(balance_wei, "ether")))
...
def codegen(
    ...
    save: bool = typer.Option(True, "--save/--no-save", help="Salvar código gerado em generated/."),
) -> None:
    ...
    if save:
        ...
        arquivo = output_dir / f"{ts}_{slug}.py"

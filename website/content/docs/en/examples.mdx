...
description: "Complete examples of real applications using Arc DevKit."
section: "Guides"
order: 31
---
...
Structure of a complete payment bot:
...
copilot = DevCopilot(offline=True)  # offline in production without AI debug
...
Full FastAPI server with WebSocket:
...
async def root():
    ...
    <body>
        ...
        <div id="events"></div>
        <script>
            const ws = new WebSocket("ws://localhost:8000/monitor/0xYourWallet")
            ws.onmessage = (e) => {
...
## Batch transaction analysis script
...
## Monitoring agent with Telegram
...
def on_event(event):
    if event["event_type"] == "native":
        direction = "Received" if event["type"] == "credit" else "Sent"
        wei = event["change_wei"]
        arc_value = int(wei) / 10**18
        send_telegram(
            f"{direction}: {arc_value:.4f} ARC\n"
            f"Wallet: {event['address'][:8]}..."
        )


monitor = MonitorAgent(
    watched_addresses=["0xYourWallet"],
    interval_seconds=30,
    webhook_url=None,  # using direct callback
)
